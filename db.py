...
"""
db.py — PostgreSQL connection for IPL NEXUS (Streamlit Cloud + Supabase)

- Uses st.secrets for production
- Uses environment variables for local
- No connection pooling (avoids stale connection issues)
"""
...
# ─────────────────────────────────────────────────────────────
# Get DB config (Streamlit secrets → fallback to env)
# ─────────────────────────────────────────────────────────────
def _get_db_config() -> dict:
    try:
        secrets = st.secrets["postgres"]
...
# ─────────────────────────────────────────────────────────────
# Create fresh connection (no pooling)
# ─────────────────────────────────────────────────────────────
def get_connection():
...
def run_query(sql: str) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    ...
    try:
        ...
        # If no result (e.g., INSERT/UPDATE)
        if cur.description is None:
            conn.commit()
            return pd.DataFrame(), None
...
# ─────────────────────────────────────────────────────────────
# Test connection (used in UI button)
# ─────────────────────────────────────────────────────────────
def test_connection() -> bool:
    df, err = run_query("SELECT 1 AS ok")
    return err is None
...
# ─────────────────────────────────────────────────────────────
# Schema context for AI (unchanged)
# ─────────────────────────────────────────────────────────────
SCHEMA_CONTEXT = """
PostgreSQL database: postgres (Supabase)
...
CRITICAL DATA FACTS:
- is_wicket stores 't' or 'f' (STRING)
- over_number range is 1 to 20
- player_of_match format is {"Name"}
"""