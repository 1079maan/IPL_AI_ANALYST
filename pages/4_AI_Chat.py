...
# # ── 15 Verified questions with real answers from dataset ──────────────────────
# VERIFIED_QUESTIONS = {
#     "🏏 Batting": [
#         ("Top 10 batsmen all time",          "V Kohli leads with 8,661 runs"),
#         ("Most sixes in IPL history",        "CH Gayle hit 357 sixes"),
#         ("Most fours in IPL history",        "V Kohli hit 772 fours"),
#         ("Highest strike rate batsmen",      "Min 500 runs scored"),
#         ("Highest team score ever",          "SRH scored 287 runs"),
#         ("Win rate batting first vs second", "Batting first wins 44.9%"),
#     ],
#     "🎯 Bowling": [
#         ("Top wicket takers in IPL",         "YS Chahal leads with 219 wickets"),
#         ("Best economy rate bowlers",        "Min 10 matches played"),
#         ("Death over statistics",            "Overs 16–20 run rate 9.52"),
#         ("Probability of hitting six in first two overs", "2.26% per ball"),
#     ],
#     "🏆 Teams": [
#         ("Which team has most wins",         "Mumbai Indians with 151 wins"),
#         ("Toss impact on results",           "Field first wins 53.8%"),
#         ("Runs per season trend",            "2008 to 2025 growth"),
#     ],
#     "⭐ Awards": [
#         ("Player of match awards",           "AB de Villiers won 25 awards"),
#         ("Most matches played at each venue","Eden Gardens hosted 77 matches"),
#     ],
# }
...
#         "🏟️  Most matches played at each venue",
#     ]
#     for s in QUICK:
#         if st.button(s, key=f"q_{s}"):
#             st.session_state.pending_q = s.split("  ", 1)[-1]
...
#     # Bottom hint
#     # ── Verified Questions Section ─────────────────────────────────────────
#     st.markdown('<div class="sec-head">✅ Verified Questions — Click to Ask Instantly</div>',
#                 unsafe_allow_html=True)
#     st.markdown("""
#     <div style="background:rgba(0,240,255,.04);border:1px solid var(--border-c);
#                 border-radius:8px;padding:.8rem 1.2rem;margin-bottom:.8rem;">
#         <div style="font-family:'Barlow',sans-serif;font-size:.84rem;color:var(--muted2);line-height:1.7;">
#             These questions are <b style="color:var(--cyan);">verified to work</b> with your IPL dataset.
#             Click any question to get an instant answer with a chart.
#         </div>
#     </div>
#     """, unsafe_allow_html=True)

#     for category, questions in VERIFIED_QUESTIONS.items():
#         st.markdown(f'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:.72rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--orange);margin:.8rem 0 .4rem;">{category}</div>', unsafe_allow_html=True)
#         qcols = st.columns(2)
#         for i, (q, hint) in enumerate(questions):
#             with qcols[i % 2]:
#                 if st.button(f"▶  {q}", key=f"vq_{q}", help=hint):
#                     st.session_state.pending_q = q

#     st.markdown('<div class="hint-bar">↓ &nbsp; Or type your own question below &nbsp; ↓</div>',
#                 unsafe_allow_html=True)
#     st.markdown('</div>', unsafe_allow_html=True)
